...
// Computed property to check if there is any product out of stock
        ...
        <div v-else>
            <div v-for="item in cartStore.cartItems" :key="item.id" class="flex items-center border-b py-4">
                ...
                <div v-if="item.product.stock == 1" class="flex items-center space-x-2">
                    <button @click="decreaseQuantity(item, item.quantity, item.type_price)"
                ...
                <button @click="cartStore.removeItem(item), deleteItemMessage()"
                    class="text-red-500 ml-4">Eliminar</button>
            </div>

            <div class="mt-6 text-right">
                <h3 class="text-xl font-bold">Total: ${{ cartStore.cartTotal }}</h3>
            </div>
            <!-- Modal button, disabled if any item is out of stock -->
