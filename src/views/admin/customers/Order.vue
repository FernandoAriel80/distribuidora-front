          ...
          <tbody>
            <tr v-for="order in orders" :key="order.id">
              <td class="border border-gray-200 p-2">
                {{ formatDate(order.hour_and_date) }}
              </td>
              ...
              <td class="border border-gray-200 p-2">
                {{ order.card_last_numb ? "***-" + order.card_last_numb : "Sin Tarjeta" }}
              </td>
              <td class="border border-gray-200 p-2">
                {{ order.card_name_user ? order.card_name_user : "Sin Tarjeta" }}
              </td>
              <td
              ...
              <td
                ...
                class="border border-gray-200 p-2 text-red-500"
              >
                {{ order.status }}
              </td>
              <td
                v-if="order.delivery_status == 'Listo'"
                class="border border-gray-200 p-2 text-green-500"
              >
                {{ order.delivery_status }}
              </td>
              <td
                v-if="order.delivery_status == 'Revision'"
                class="border border-gray-200 p-2 text-orange-500"
              >
                {{ order.delivery_status }}
              </td>
              <td
                v-if="order.delivery_status == 'Entregado'"
                class="border border-gray-200 p-2 text-blue-500"
              >
                {{ order.delivery_status }}
              </td>
              <td
                class="border border-gray-200 p-2 text-blue-500 cursor-pointer"
              ...
              <td class="border border-gray-200 p-2">
                <select
                  ...
                  class="border p-1"
                  @change="
                    updateOrderStatus(order.id, order.status, order.delivery_status)
                  "
                >
